...
public:
	...
	Chunk(Noise& noise, glm::ivec2 worldPosition, bool wireFrame = false) :
			worldPos(worldPosition)
	{
		chunkSeed = noise.desc.seed * worldPos.x + worldPos.y;
		constexpr int borderedSize = DefaultTrisPerSide+1 + (DefaultBorderWidth*2); // +1 for verts/side, +2 for 1 border on each side
		HeightMap<borderedSize> heightMap = heightMap.generate(noise, {worldPos.x * DefaultTrisPerSide, worldPos.y * DefaultTrisPerSide}, 0.0f, 1.0f);

		Grid<int, borderedSize> borderedIndices = Grid<int, borderedSize>::generateBorderedGrid(1);

		mesh = Mesh::generate<borderedSize, DefaultBorderWidth>(heightMap, {worldPos.x * DefaultTrisPerSide, worldPos.y * DefaultTrisPerSide}, wireFrame);
...
//		mesh = Mesh(heightMap, borderedIndices, borderedSize, chunkSize + 1, wireFrame);
	...
	static constexpr int DefaultTrisPerSide = 4;
		...
		for (auto &p: pois) {
			...
			for (int row = -p.distance; row <= p.distance; row++) {
				for (int col = -p.distance; col <= p.distance; col++) {
					...
					glm::ivec2 currentChunkPos = p.center + glm::ivec2{row, col};
...
public:
	...
	explicit Terrain(Noise::Descriptor noiseDesc, glm::ivec2 centerChunkPos = DefaultCenter, int numVisibleChunks = DefaultLoadDistance, int chunkSize = Chunk::DefaultTrisPerSide, bool wireFrame = false);
